...
void join_network(const uint8_t maxAttempts)
{
    int32_t ret = mDot::MDOT_ERROR;

    // attempt to join the network
    for (int32_t j_attempts = 0; j_attempts < maxAttempts; j_attempts++)
    {
        logInfo("attempt %d to join network", j_attempts + 1);
        ret = dot->joinNetwork();
        if (ret != mDot::MDOT_OK)
        {
...
void sleep_configure_io()
{
    ...
    GPIO_InitStruct.Pin = GPIO_PIN_2 | GPIO_PIN_6 | GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_12 | GPIO_PIN_13 | GPIO_PIN_14 | GPIO_PIN_15;
    GPIO_InitStruct.Mode = GPIO_MODE_ANALOG;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
